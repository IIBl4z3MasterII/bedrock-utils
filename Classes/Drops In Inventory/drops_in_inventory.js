...
╚═════╝ ╚══════╝╚═╝╚══════╝╚═════╝     ╚═╝     ╚═╝╚═╝  ╚═╝╚══════╝   ╚═╝   ╚══════╝╚═╝  ╚═╝
          ...
          Drops In Inventory •  By: @bl4z3master
*/
...
class InventoryManager {
    ...
    clamp(amount) {
        ...
        if (amount >= 256) return this.#MAX_AMOUNT;
        return amount;
    }
...
class ItemCollector {
    ...
    constructor() {
        this.#registry = new BlockBreakRegistry();
    ...
    init() {
        ...
        world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
            if (initialSpawn) this.#deliverOverflow(player);
        });
    }

    #onBlockBreak({ player, block }) {
        this.#registry.register(block, player);
    }
    ...
    #collectItem(entity, player) {
        try {
            ...
            if (this.#inventory.addItem(player, original)) {
                entity.kill();
                ...
                entity.kill();
                player.sendMessage(
                    "§eInventario lleno. §fEl item fue guardado y se entregará cuando tengas espacio."
                );
            }
        } catch (error) {
    ...
    #saveOverflow(player, item) {
        overflowDB.onReady(() => {
            try {
                ...
                let pending = [];
                if (overflowDB.has(key)) {
                    const stored = overflowDB.get(key);
                    pending = Array.isArray(stored) ? stored : (stored ? [stored] : []);
                }
                ...
                console.log(
                    `§eVaultDB > Item overflow guardado para [${player.name}]. ` +
                    `Pendientes: ${pending.length}`
                );
            } catch (err) {
                console.error(`[❌ VaultDB overflow save error] ${err}`);
            }
        });
    }
        ...
        overflowDB.onReady(() => {
            try {
                ...
                for (const item of pending) {
                    if (!this.#inventory.addItem(player, item)) {
                ...
                if (undelivered.length === 0) {
                    overflowDB.delete(key);
                    player.sendMessage(
                        `§a✅ Se entregaron todos tus items guardados (${pending.length}).`
                    );
                } else {
                    overflowDB.set(key, undelivered);
                    player.sendMessage(
                        `§e⚠ Se entregaron ${pending.length - undelivered.length} items. ` +
                        `§f${undelivered.length} siguen guardados (inventario lleno).`
                    );
                }
            } catch (err) {
                console.error(`[❌ VaultDB overflow deliver error] ${err}`);
            }
        });
    }
}
