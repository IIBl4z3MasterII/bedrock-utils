...
/* ============================
   DynamicStore — target-agnostic dynamic property wrapper
   (works with world or entity as target)
   ============================ */
class DynamicStore {
  #target;
  #ns;
    ...
    const stored =
      typeof value === "object" && value !== null
        ...
        : typeof value === "boolean"
          ...
          : value;
    try {
      this.#target.setDynamicProperty(key, stored);
    } catch (error) { this.log(`Error setting dynamic property ${key}: ${error}`, true); }
  ...
  /* Chunked storage for payloads > ~8 KB */
  setLarge(name, value, chunkSize = 8000) {
    ...
    for (let i = 0; i < numChunks; i++) {
      this.set(`${name}_chunk${i}`, str.slice(i * chunkSize, (i + 1) * chunkSize));
    }
  }
  ...
  getLarge(name, defaultValue = undefined) {
    ...
    if (typeof meta === "string") {
      try { return JSON.parse(meta); } catch { return meta; }
    }
    if (typeof meta === "number" && meta > 0) {
      let result = "";
      for (let i = 0; i < meta; i++) {
        result += this.get(`${name}_chunk${i}`, "");
      }
...
/* ============================
   WorldManager — lifecycle + store factories
   ============================ */
class WorldManager {
  constructor() {
    ...
    const runInit = () => {
      ...
      for (const cb of this.#readyQueue) {
        try { cb(); } catch (error) { this.log(`Error in onReady callback: ${error}`, true); }
      }
  ...
  store(namespace) {
    if (!this.#stores.has(namespace)) {
      this.#stores.set(namespace, new DynamicStore(world, namespace, true));
    }
    return this.#stores.get(namespace);
  }

  entityStore(entity, namespace) {
    return new DynamicStore(entity, namespace, false);
  }

  rawScan(predicate) {
    return world.getDynamicPropertyIds().filter(predicate);
  }
  ...
  onReady(callback) {
    if (this.worldLoaded) callback();
    else this.#readyQueue.push(callback);
  }

  registerInitFunction(func, message) {
    if (this.worldLoaded) {
      try { func(); if (message && this.debugMode) this.log(message); } catch (error) { this.log(`Error in init function: ${error}`, true); }
      return;
    }
  ...
  log(message, isError = false) {
    if (isError) logger.error("WorldManager", message);
    else if (this.debugMode) logger.debug("WorldManager", message);
  }
  ...
  getProperty(propertyName, namespace = "") {
    ...
    if (typeof raw !== "string") {
      ...
      const n = Number(raw);
      parsed = !isNaN(n) && raw !== "" && Number.isSafeInteger(n) && String(n) === raw ? n : raw;
    }
