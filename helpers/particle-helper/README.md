...
Opciones comunes a `line`, `circle`, `circleVertical`, `sphere`,
`cubeOutline`, `perimeter`, `trail`:

- `options.player`: si se pasa, solo spawnea partículas dentro de
  `options.maxDistance` bloques del jugador (optimización para no
  renderizar formas grandes completas si el jugador está lejos de una
  parte). Si no pasás `options.maxDistance` pero sí `options.player`,
  usa un default de `18` bloques — antes no había default y sin
  `maxDistance` explícito el filtro quedaba desactivado.
- El chequeo de distancia (`#inRange`) ahora es 3D (incluye eje Y),
  antes solo comparaba X/Z.
