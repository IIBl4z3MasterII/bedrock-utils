...
| Archivo | Rol |
|---|---|
| `index.js` | Funciones `action`, `modal`, `message`, `buildForm`, `buildAndShow`, `registerActionMenu`, `registerModalForm`, `mostrarMenu`, `mostrarModal` |
...
| `buildAndShow(tpl, ctx?)` | igual que arriba, `ctx.player` requerido | `Promise<FormResponse>` | Arma el form y lo muestra a `ctx.player` |
...
| `mostrarMenu(player, menuId, onBack?)` | — | Muestra el menú registrado; si se cancela, llama a `onBack` |
| `mostrarModal(player, formId, onBack?)` | — | Muestra el modal registrado; si se cancela, llama a `onBack` |
...
- `mostrarMenu`/`mostrarModal` tragan errores de `.show()` con un
  `.catch(() => {})` silencioso (ej. si el jugador se desconecta a mitad
  de la animación del form) — si necesitás loguear esos casos, hacer tu
  propio wrapper alrededor.
