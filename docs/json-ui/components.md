...
### 3.2 Image with UV (sprite atlas) — AmBro animated background
```json
"ambro_bg_anim": {
  "type": "image",
  "texture": "textures/ui/ambro/xaambro_background",
  "size": ["100%", "100%"],
  "uv": [0, 0],                    // [x, y] in the texture
  "uv_size": [128, 128],           // [w, h] of the crop
  "animations": [{
    "anim_type": "uv_offset",
    "from": [0, 0],
    "to": [0, 128],
...
```
Scoreboar/System Slayer `_AmBro/animations.json` — animated background moving the UV of a sprite.

### 3.3 Image with `flip_book` (animated sprite sheet) — Scoreboar loading_anim.json
```json
"anim": {
  "anim_type": "flip_book",
  "initial_uv": [0, 0],
...
### 10.1 Player renderer in chest (UltimateSu chest_screen.json)
```json
"rr_player_renderer": {
  "type": "custom",
  ...
  "anchor_to": "center",
  "renderer_bounds": {
    "L": -0.5, "R": 0.5, "T": -0.5, "B": 0.5, "P": 0.5, "Q": -0.5
  },
  "pivot": { "x": 0, "y": 0, "z": 0 },
  "rotation": { "x": 0, "y": 0, "z": 0 }
}
```
- `live_player_renderer` — renders the real player (3D) in the UI.
- `renderer_bounds` / `pivot` / `rotation` control the view.

### 10.2 Paperdoll with renderer_hover_text (bl4z3_forms)
```json
"renderer_hover_text": {
  "type": "custom",
