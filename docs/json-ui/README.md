...
| **Containers (Chest)** | `chest_screen.json` same-key override, 9×N grids, `common.inventory_screen_common` inheritance | ✓ Complete (invsee) |
| **Pause Screen** | `insert_back` modifications in `root_panel`, custom button + waypoints panel | ✓ Functional (WayMar, ChunkPrevi) |
| **Animations** | `anim_type`: alpha/size/offset/wait/flip_book, `easing`, `next`, `destroy_at_end`, `clip_direction:left` for bars | ✓ Rich (Scoreboar loading, bl4z3_forms gradient) |
...
| `ZhkTZFZlAgA=` | `[RP] System Slayer v2` | Scoreboard, server_form, main screen overrides (start, settings, play) |
| `H3o4ZE1zCQA=` | `[Scoreboar]` (variant) | Main screen overrides + scoreboard |
| `48mGwha4hgQ=` | `pack(2)` (RP+BP) | 116KB giant shop, animated_bar, simple server_form |

> Total: **10 worlds with real UI** from the 18 worlds of **IIBl4z3MasterII**.