...
### 6.1 Full override (UltimateSu chest_screen.json:1-45)
```json
{
  "namespace": "chest",
  "chest_screen@common.inventory_screen_common": {
    "chest_label": { "text": "$container_title", "color": "$title_text_color" },
    "small_chest_grid": {
      "type": "grid",
      "size": [162, 54],
      "anchor_from": "top_left",
...
### 6.2 Item template (UltimateSu chest_screen.json:46-68)
The item is not rewritten by hand: you inherit the vanilla `common.container_item` template (which internally uses the `inventory_item_renderer` renderer with a collection binding) and only link the correct collection:
```json
...
```
> The vanilla `common.item_renderer` renderer (`type: "custom"`, `renderer: "inventory_item_renderer"`) draws the item icon with its own texture (from the item atlas), not via an `image` node with `textures/ui/...`.

### 6.3 `pocket_containers.json` override (touch) — invsee
```json
"small_chest_grid": {
  "type": "grid",
  "size": [162, 54],
