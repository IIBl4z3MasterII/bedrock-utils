...
| `collection_details` | Reads details of the current item (icon, count, custom_name) | container grids (vanilla `common.container_item`) |
...
```
The engine exposes `#item_icon`, `#count`, `#custom_name`, `#item_id` of the item in that cell. The vanilla `common.container_item` template uses this pattern.
