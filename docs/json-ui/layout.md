...
|---|---|---|
| `100` | Absolute pixels | `"size": [162, 54]` (UltimateSu chest 9×3) |
...
### 5.1 Basic grid (UltimateSu chest_screen.json:50-68)
```json
"small_chest_grid": {
  "type": "grid",
  "size": [162, 54],
  ...
  "anchor_to": "top_left",
  "grid_dimensions": [9, 3],           // [columns, rows]
  "grid_item_template": "chest.chest_grid_item",
  "collection_name": "container_items"
}
```

### 5.2 Grid with factory + rescaling (bl4z3_forms form.json:47-91)
```json
"form_grid": {
  "type": "grid",
  "grid_dimensions": [3, 3],
...
### 5.3 `grid_rescaling_type` observed
| Value | Behavior |
|---|---|
| `"horizontal"` | Adds columns, fixed rows (bl4z3_forms, Scoreboar shop) |
| `"vertical"` | Adds rows, fixed columns |
| `"none"` | Fixed dimensions (UltimateSu chest) |

### 5.4 Grid with `collection_index` — static positioning (bl4z3_forms main.json:91-141)
```json
"top": {
  "type": "stack_panel",
  "size": ["100%c", "50%"],
