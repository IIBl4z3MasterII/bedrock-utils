...
## 3. `rp invsee` — `jt+WZgXoAQA=` (world `1_7`)
...
## 4. `[RP]WayMar` — `Geq7734AhFM=` (world `Waypoint`)
...
## 5. `RP ChunkPrevi` — `TuRCdiIjjEw=` (world `ChunkPreviews`)
...
## 6. `[RP]ChatCh` — `Ckr82bjcBWA=` (world `ChatChannels`)
...
## 7. `Leaderboar` — `ehtcdHnEp4o=` (world `Leaderboards`)
...
## 8. `[RP] System Slayer v2` — `ZhkTZFZlAgA=` (world `System Slayer`)
...
- ✅ HUD + scoreboard + server form overrides
...
## 9. `[Scoreboar]` H3o4ZE variant — `H3o4ZE1zCQA=` (world `SkyWars`)

**Location**: `minecraftWorlds\H3o4ZE1zCQA=\resource_packs\[Scoreboar\ui\`

**Key files**:
- Same structure as yUyl6Pz but with `add_external_server_screen`, `how_to_play`, `npc_interact`, `play_screen`, `profile_screen`, `progress_screen`, `settings_screen`, `start_screen`

**Contributions**:
- ✅ Combines Scoreboar + System Slayer (scoreboard + main screens)

---

## 10. `pack(2)` — `48mGwha4hgQ=` (world `Test`)
...
## 11. `[RP]AcidRa` — `Jj1fevJ16G0=` (world `AcidRain`)
...
## 12. `Skin` — `auccjAVLQ+g=` (world `SkinTest`)
...
## 13. Worlds without UI resource packs
...
| Pattern | Scoreboar | Bl4z3 | WayMar | ChatCh | Leader | S.Slayer | pack(2) |
|---|---|---|---|---|---|---|---|
| HUD override | ✓ | ✓ | | ✓ | ✓ | ✓ | |
| Full server form | ✓ | ✓ | ✓ | | | ✓ | ✓ |
| `$title` router | | ✓ | | | | | |
...
4. **System Slayer** (ZhkTZFZlAgA) — main screen overrides + HUD/scoreboard
5. **ChunkPrevi** (TuRCdiIjjEw) — gallery_form
6. **ChatCh** (Ckr82bjcBWA) — chat with channels
7. **pack(2)** (48mGwha4hgQ) — giant shop (scale reference)
8. **Leaderboar** (ehtcdHnEp4o) — leaderboard HUD
9. **H3o4ZE** — Scoreboar + S.Slayer combination
10. **AcidRa / Skin** — minimal