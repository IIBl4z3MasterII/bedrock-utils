...
- `systems/` — 5 complete systems with event listeners and persistence: `ban-system`, `death-custom-msg`, `drops-in-inventory` (+ `VaultDB`), `mob-stacker` (mob stacker via dynamic properties), `world-manager` (`DynamicStore` + `WorldManager`). Barrel export and `systems/index.js`.
